...
const getCoordinatesFromTail = (
  ...
  if (vertical) {
    for (let i = 0; i < shipLength; i++) {
      coordinates.push([tail[0] - i, tail[1]]);
    }
  } else {
    for (let i = 0; i < shipLength; i++) {
      coordinates.push([tail[0], tail[1] - i]);
    }
  }
  return coordinates;
};

const addClassToCells = (
  cells: NodeList,
  coordinates: number[][],
  classToAdd: string
): HTMLElement => {
  cells.forEach((node) => {
    ...
    coordinates.forEach((coordinate) => {
      if (
        parseInt(cell.dataset.row) === coordinate[0] &&
        parseInt(cell.dataset.col) === coordinate[1]
      ) {
...
const readPlayerInput = () => {
  ...
  const cells = document.querySelectorAll(
    ".placeShips .container .grid .dropzone"
  ) as NodeList;
  ...
  interact(".dropzone").dropzone({
    ...
    ondragenter: function (event) {
      ...
      let tail = [
        parseInt(dropzoneElement.dataset.row),
        parseInt(dropzoneElement.dataset.col),
      ];
      ...
      // check for overlap
      const occupied = Object.entries(activeCoordinates).filter(
        ([key, value]) => key !== ship
      );
      let overlap = false;
      occupied.forEach((occupiedArea) => {
        for (let j = 0; j < coordinates.length; j++) {
          for (let i = 0; i < occupiedArea[1].length; i++) {
            if (
              occupiedArea[1][i][0] == coordinates[j][0] &&
    ...
    ondrop: function (event) {
      ...
      let coordinates = getCoordinatesFromTail(tail, shipLength, shipVertical);
      // check for overlap
      let occupied = Object.entries(activeCoordinates).filter(
        ([key, value]) => key !== ship
      );
      let overlap = false;
      occupied.forEach((occupiedArea) => {
      ...
      for (let i = 0; i < 5; i++) {
        starts.push(
          Object.values(activeCoordinates)[i][
            Object.values(activeCoordinates)[i].length - 1
          ]
        );
      }
    },
  ...
  interact(".ship")
    .on("doubletap", function (event) {
      ...
      shipDirection[event.currentTarget.dataset.ship] =
        !shipDirection[event.currentTarget.dataset.ship];
